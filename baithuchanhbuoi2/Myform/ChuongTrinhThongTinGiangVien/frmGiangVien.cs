...
namespace ChuongTrinhThongTinGiangVien
{
    public partial class frmGiangVien : Form
    {
        ...
        {
            int i =lbHocPhanDay.SelectedItems.Count-1;
            while(i>=0)
            {
        ...
        public GiangVien GetGiangVien()
        {
            ...
            gv.SoDT=this.mtxtDT.Text;
            string ngoaiNgu = "";
            for (int i = 0; i < chkNgoaiNgu.Items.Count; i++)
                if(chkNgoaiNgu.GetItemChecked(i))
                    ngoaiNgu += chkNgoaiNgu.Items[i]+";";
            gv.NgoaiNgu = ngoaiNgu.Split(';');
            DanhSachMucHocPhan dshp = new DanhSachMucHocPhan();
